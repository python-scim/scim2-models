...
@pytest.mark.parametrize(
    "expression",
    [
        'emails.type[type eq "work"]',
        'urn:ietf:params:scim:schemas:core:2.0:User:emails.type[type eq "work"]',
        'emails.type[type eq "work"] and userName pr',
    ],
)
def test_sub_attribute_before_value_path_is_rejected(expression):
    """``valuePath = attrPath "[" valFilter "]"`` lets the attrPath name a sub-attribute.

    The filter between the brackets addresses the sub-attributes of the
    selected attribute, and a sub-attribute has none of its own, so the form
    is left without a meaning to give it.
    """
    with pytest.raises(
...
def test_rendering_a_value_no_filter_can_carry_is_rejected():
    node = Comparison(AttrPath("userName"), CompareOperator.eq, object())
    with pytest.raises(TypeError, match="not a value a filter can carry"):
        str(node)
...
@pytest.mark.parametrize(
    ...
    [
        ("bjensen", '"bjensen"'),
        (
            'x" or userName pr or userName eq "y',
            r'"x\" or userName pr or userName eq \"y"',
        ),
...
def test_an_attribute_path_refuses_a_sub_attribute_that_is_not_a_name():
    with pytest.raises(ValueError, match="is not an attribute name"):
        AttrPath("emails", sub_attr='type eq "work"')


def test_an_attribute_path_refuses_a_uri_that_is_not_a_urn():
    with pytest.raises(ValueError, match="is not a schema URN"):
        AttrPath("userName", uri="https://example.com/schemas/User")
