...
def test_a_bare_comparison_path_answers_a_filter_over_the_entries():
    """Errata 7122 allows ``attrExp`` as a path, which selects entries by their value.

    The filter is expressed against an entry whichever spelling the path used,
    so a consumer applies it without having to rebase it.
    """
    assert str(Path('schemas eq "urn:x:y"').value_filter) == 'value eq "urn:x:y"'
    assert str(Path('emails.type eq "work"').value_filter) == 'type eq "work"'


def test_a_bare_presence_path_answers_a_presence_over_the_entries():
    """A presence test is rebased the same way a comparison is."""
    assert str(Path("title pr").value_filter) == "value pr"
...
def test_a_quoted_value_cannot_be_read_as_path_syntax(user):
    untrusted = 'work" or type pr'
    path = Path[User](f"emails[type eq {ScimFilter.quote(untrusted)}].value")
    assert str(path.value_filter) == r'type eq "work\" or type pr"'
...
def test_a_bare_presence_selects_every_assigned_value():
    """A bare presence selects every value, and ``schemas`` is emptied rather than unset.

    An attribute left without a value becomes :data:`None`, which the annotation
    of ``schemas`` does not allow.
    """
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param('emails.type eq "work"', "emails", id="bare comparison"),
...
def test_the_two_spellings_of_one_selection_designate_the_same_attribute(user):
    """``emails.type eq "work"`` selects the same values as ``emails[type eq "work"]``.

    The sub-attribute of the bare form belongs to the filter rather than to the
    attribute the path operates on, so the introspection answers on ``emails``
    for both, which is what reading through either already returned.
    """
    bracketed = Path[User]('emails[type eq "work"]')
    bare = Path[User]('emails.type eq "work"')
...
def test_the_annotations_of_a_bare_comparison_come_from_the_attribute_it_selects():
    """``members`` is not case-exact where the ``members.value`` it filters on is.

    Reading the annotations off the filtered sub-attribute would describe an
    attribute the path does not operate on.
    """
