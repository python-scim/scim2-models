...
def test_resolution_of_a_qualified_path_against_a_complex_attribute_raises():
    """A complex attribute declares no schema of its own to qualify against.

    §3.10 has a URN qualify the name of an attribute a schema declares, and the
    sub-attributes it holds are reached through the dot notation that follows
...
def test_a_quoted_value_cannot_be_read_as_syntax():
    untrusted = 'x" or userName pr or userName eq "y'
    scim_filter = ScimFilter[User](f"userName eq {ScimFilter.quote(untrusted)}")
...
def test_a_qualified_attribute_within_a_value_selection_is_not_silently_ignored():
    """A URN inside the brackets designates nothing, and used to resolve to nothing.

    The filter kept evaluating, against an attribute it had failed to bind, so
    it never matched anything.
    """
    expression = f'emails[{User.__schema__}:value eq "bjensen@example.com"]'
    scim_filter = ScimFilter[User](expression)
    assert not scim_filter.match(
        User(user_name="bjensen", emails=[{"value": "bjensen@example.com"}])
    )

    with pytest.raises(InvalidFilterException) as exc_info:
        scim_filter._validate_semantics()
    assert exc_info.value.scim_type == "invalidFilter"
    assert "cannot qualify" in str(exc_info.value)
...
def test_resolution_skips_extensions_that_do_not_match_the_urn():
    ...
    class SuperHero(Extension):
        __schema__ = "urn:example:extensions:SuperHero"
