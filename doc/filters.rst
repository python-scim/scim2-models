...
:attr:`~scim2_models.ScimFilter.ast` is the parsed tree, and
...
query: its syntax when it is created, then
:meth:`~scim2_models.ScimFilter.validate_semantics` for the attributes it names and the values
it compares them to. What follows assumes a filter that passed both, which is what lets a
...
before emitting anything: :meth:`~scim2_models.ScimFilter.resolve_comparison` for a
comparison, :meth:`~scim2_models.ScimFilter.resolve` for a presence test or a value
...
:meth:`~scim2_models.ScimFilter.match` walks the same tree through the same resolution, on
...
.. list-table::
   ...
   * - `6001 <https://www.rfc-editor.org/errata/eid6001>`_,
     ...
     - Reference attributes are declared ``caseExact: false`` while §2.3.7 requires them to be
       ...
       :attr:`~scim2_models.CaseExact.true`, which :meth:`~scim2_models.Resource.to_schema`
       reflects.
