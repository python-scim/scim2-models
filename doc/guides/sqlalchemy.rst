...
SCIM resource and keep the ones :meth:`~scim2_models.ScimFilter.match` accepts, which reads the
...
:meth:`~scim2_models.ScimFilter.resolve_comparison` turns one into the field it designates on
...
- **case sensitivity**, read from
  :attr:`~scim2_models.filters.ResolvedAttribute.case_exact` and the type of the attribute.
...
:attr:`~scim2_models.SearchRequest.sort_by` is a :class:`~scim2_models.Path`, resolved through
the same table of columns, which is also what rejects sorting on an attribute stored in another
table.
