...
// where it has to stand as a boolean expression of its own. The ABNF lets the
// attrPath before the brackets carry one too, which the builder refuses: the
// filter between them addresses sub-attributes of that attribute, and SCIM
...
// The patterns are the ones AttrPath checks its own names and URN against.
ATTR_PATH: /(?:{_URN}:)?{_ATTR_NAME}(?:\.{_ATTR_NAME})?/i
SUB_ATTR: /{_ATTR_NAME}/
...
def _select_attr_path(raw: str) -> AttrPath:
    ...
    if attr_path.sub_attr is not None:
        raise ValueError(
            f"a value selection cannot apply to the sub-attribute {str(attr_path)!r}"
        )
    return attr_path
...
class _AstBuilder(Transformer[Token, Any]):
    ...
    def value_path(self, attr_path: Token, val_filter: FilterNode) -> FilterNode:
        return ValuePath(
            attr_path=_select_attr_path(str(attr_path)), val_filter=val_filter
        )
