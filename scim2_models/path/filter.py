...
from .grammar import _parse_filter
...
class _Validator(FilterVisitor[None]):
    ...
    def _resolve(
        ...
        attribute only some of them declare, since RFC7644 §3.4.2.1 has the
        others evaluate it to false. Resolution therefore fails only when no
        model declares it.
        """
    ...
    def visit_value_path(self, node: ValuePath) -> None:
        ...
        _validate_value_filter(resolved, node.val_filter, strict=self.strict)


def _validate_value_filter(
    ...
    is left alone. A filter naming an attribute the selected model does not
    declare, or comparing one in a way it does not accept, raises
...
class ScimFilter(_BoundToModels, _Expression, Generic[ResourceT]):
    ...
    def __new__(
        ...
        filter_ = super().__new__(cls, _text(expression))
        ...
        filter_._ast = (
            ...
            else _parse_filter(str(filter_))
        )
    ...
    def quote(value: Any) -> str:
        r"""Render a value as a literal a filter can carry, quotes included.

        A string is quoted and escaped, so that it stays a value whatever it
        contains. A number, a boolean, :data:`None` or a
        :class:`~datetime.datetime` render as the syntax their attribute type
        expects. The literal fills an unquoted slot of the expression::

            >>> from scim2_models import ScimFilter
            >>> untrusted = 'x" or userName pr'
            >>> print(ScimFilter.quote(untrusted))
            "x\" or userName pr"
            >>> print(ScimFilter(f"userName eq {ScimFilter.quote(untrusted)}"))
            userName eq "x\" or userName pr"
            >>> ScimFilter.quote(True), ScimFilter.quote(18), ScimFilter.quote(None)
            ('true', '18', 'null')
            >>> from datetime import datetime, timezone
            >>> ScimFilter.quote(datetime(2011, 5, 13, 4, 42, 34, tzinfo=timezone.utc))
            '"2011-05-13T04:42:34+00:00"'
    ...
    def _single_model(self, method: str) -> "type[BaseModel] | None":
        ...
        A filter bound to a union has no single model to resolve against, and
        raises TypeError. A transpiler emits one query per resource type, so it
        binds the filter to each of them in turn.
        """
    ...
    def resolve(
        ...
        if (model := self._single_model("resolve")) is None:
            return None
        return _resolve_filter_path(model, attr_path, strict=strict)
    ...
    def resolve_comparison(
        ...
        if (model := self._single_model("resolve_comparison")) is None:
            return None
        return _resolve_filter_path(
            model, attr_path, strict=strict, for_comparison=True
        )
    ...
    def _validate_semantics(self, *, strict: bool = True) -> None:
        ...
        scim2_models.SearchRequest turns on: naming the resource types an
        endpoint serves is what makes an attribute none of them declares a
        client error, where RFC7644 §3.4.2.1 has an unparameterised request
        evaluate it to false instead.

        Under ``strict``, an unknown attribute raises InvalidFilterException,
        as does a value or an operator that is not valid for its attribute.
        """
    ...
    def match(self, resource: ResourceT, *, strict: bool = False) -> bool:
        ...
        return _Evaluator(model, resource, strict=strict).visit(self.ast)
