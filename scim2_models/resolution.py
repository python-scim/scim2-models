...
def _target_model(
    ...
    # RFC7644 §3.10 has a URN qualify the name of an attribute a schema
    # declares, and the ABNF of §3.4.2.2 has the path inside a value selection
    # name a sub-attribute of the attribute it selects from. A complex
    # attribute declares no schema of its own, so nothing resolved against one
    # can be qualified.
    if not (isclass(model) and issubclass(model, Resource | Extension)):
        raise PathNotFoundException(
            path=str(attr_path),
            detail=f"'{attr_path.attr}' is a sub-attribute, which the schema "
            f"'{attr_path.uri}' cannot qualify",
        )
